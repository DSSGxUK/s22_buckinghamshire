...
def df_unique(df, filter_len=None, dropna=False):
    unique_values = {x: df[x].value_counts(dropna=dropna).sort_values(ascending=False) for x in df.columns}
    for x in df.columns:
        if all(isinstance(y, str) for y in unique_values[x].index):
            unique_values[x + '_len'] = pd.Series(unique_values[x].index).apply(len).value_counts()
