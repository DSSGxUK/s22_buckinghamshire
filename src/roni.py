...
def calculate_roni_scores(df, threshold=None):
    ...
    #roni_df[UPN] = df[UPN]
    roni_df['roni_att_below_90'] = np.where((df['total_absences']>0.1) & (df['total_absences']<=0.15),1,0)
    roni_df['roni_att_below_85'] = np.where(((df['total_absences']>0.15) & (df['total_absences']<=1.)),2,0)
    roni_df['roni_eal'] = np.where((df[d.to_categorical('language', 'eng')]==0)&(df[d.to_categorical('language', 'enb')]==0),1,0)
    roni_df['roni_ehcp'] = np.where((df[d.to_categorical('sen_provision', 'e')]==1)|(df[d.to_categorical('sen_provision', 's')]==1)|(df[d.to_categorical('send_flag', '1')]==1)|(df[d.to_categorical('sen', 's')]==1)|(df[d.to_categorical('sen', 'e')]==1),2,0)
    roni_df['roni_sen'] = np.where((df[d.to_categorical('sen_provision', 'k')]==1)|(df[d.to_categorical('sen', 'a')]==1)|(df[d.to_categorical('sen', 'k')]==1)|(df[d.to_categorical('sen', 'p')]==1)|(df[d.to_categorical('sen_support_flag', '1')]==1),1,0) 
    roni_df['roni_excluded_1'] = np.where((df['excluded_authorised']<=0.5) & (df['excluded_authorised']>0.),1,0) 
    roni_df['roni_excluded_2'] = np.where((df['excluded_authorised']>0.5),2,0) 
    roni_df['roni_alt_provision'] = np.where((df[d.to_categorical('characteristic_code', '200')]==1),1,0) 
    roni_df['roni_lac'] = np.where((df[d.to_categorical('characteristic_code', '110')]==1), 2,0)
    roni_df['roni_pregnant_or_parent'] = np.where((df[d.to_categorical('characteristic_code', '180')]==1) | (df[d.to_categorical('characteristic_code', '120')]==1) | (df[d.to_categorical('characteristic_code', '190')]==1), 2,0)
    roni_df['roni_fsme'] = np.where((df[d.to_categorical('fsme_on_census_day', '1')]==1), 2,0)
    roni_df['roni_carer'] = np.where((df[d.to_categorical('characteristic_code', '140')]==1), 2,0)
    roni_df['roni_yot'] = np.where((df[d.to_categorical('characteristic_code', '170')]==1), 2,0)
