...
LR0 = lambda: ({ 
        ...
        'estimator__model__l1_ratio': Real(0., 1., 'uniform'),
        'estimator__model__C': Real(1e-6, 1e+2, 'log-uniform'),
        ...
        'estimator__oversampling__sampling_strategy': Real(0.1, 1., 'uniform'),  # Needs to have a lower bound bigger than % of neets and upper bound of 1.
...
SVC1 = lambda: ({
        ...
        'estimator__model__C': Real(1e-5, 1e+5, 'log-uniform'),
        'estimator__model__gamma': Real(1e-6,1e+1,'log-uniform'),
        'estimator__model__kernel': Categorical(['rbf','poly','sigmoid']),
        'estimator__model__degree': Integer(1,8,'uniform'),
        'estimator__oversampling': Categorical([RandomOverSampler()]),
        'estimator__oversampling__sampling_strategy': Real(0.1, 1., 'uniform'),  # Needs to have a lower bound bigger than % of neets and upper bound of 1.
        'estimator__oversampling__random_state': Categorical([get_random_seed()]),
        'estimator__oversampling__shrinkage': Categorical([0, 1e-4, 1e-3, 1e-2, 1e-1, 1, 2, 4]),
        'estimator__imputation': Categorical([None]),
        'estimator__scaler': Categorical([StandardScaler(), None])
}, 50)
SVC2 = lambda: ({
...
LINEAR_SVC1 = lambda: ({
        ...
        'estimator__model__loss': Categorical(['squared_hinge']), #peanlty = 'l1' and loss = 'hinge' is not supported
...
LINEAR_SVC2 = lambda: ({
        ...
        'estimator__model__loss': Categorical(['hinge', 'squared_hinge']), #peanlty = 'l1' and loss = 'hinge' is not supported
...
LGBM0 = lambda: ({
    ...
    'estimator__model__max_depth': Integer(-21,21, 'uniform'), # default  is -1, <= 0 means no depth constraint
    'estimator__model__learning_rate': Real(1e-4, 1e+2, 'log-uniform'), #0.1
    'estimator__model__min_child_samples': Integer(1,int(20 ** 2),'log-uniform'), #20
    'estimator__model__is_unbalance': Categorical([False, True]), # I think this takes into account unbalanced data?
...
LGBM1 = lambda: ({
    ...
    'estimator__model__max_depth': (-21,21, 'uniform'), # default  is -1, <= 0 means no depth constraint
    'estimator__model__learning_rate': Real(1e-4, 1e+2, 'log-uniform'), #0.1
...
LGBM2 = lambda: ({
    ...
    'estimator__model__num_leaves': (2,31**2, 'log-uniform'), #31
    'estimator__model__max_depth': (-21,21, 'uniform'), # default  is -1, <= 0 means no depth constraint
    'estimator__model__learning_rate': (1e-4, 1e+2, 'log-uniform'), #0.1
    'estimator__model__n_estimators': (5,100 ** 2,'log-uniform'), #100
    'estimator__model__min_child_samples': (2,20 ** 2,'log-uniform'), #20
    'estimator__model__is_unbalance': Categorical([False, True]), # I think this takes into account unbalanced data?
    'estimator__model__objective': Categorical(['binary']),
...
LGBM3 = lambda: ({
    ...
    'estimator__model__objective': Categorical(['binary']),
    'estimator__oversampling': Categorical([RandomOverSampler()]),
    'estimator__oversampling__sampling_strategy': (0.1, 1., 'uniform'),  # Needs to have a lower bound bigger than % of neets and upper bound of 1.
    'estimator__oversampling__random_state': Categorical([get_random_seed()]),
    'estimator__oversampling__shrinkage': Categorical([0, 1e-4, 1e-2, 1, 2, 4]),
    'estimator__imputation': Categorical([None]),
    'estimator__scaler': Categorical([StandardScaler(), None])
...
LGBM4 = lambda: ({
    ...
    'estimator__model__max_depth': Integer(-21,21, 'uniform', 'integer'), # default  is -1, <= 0 means onstraint
    'estimator__model__learning_rate': Real(1e-4, 1e+2, 'log-uniform'), #0.1
