...
from ..constants import Aggregations, ThresholdType
...
class AggregatingPostprocessor(ClassifierMixin, BaseEstimator):
    ...
    def _validate_params(self):
        ...
        if not isinstance(self.estimator, BaseEstimator):
            raise TypeError(
                f"The provided input to the estimator argument was not an estimator. You passed {self.estimator}."
            )
        ...
        if not hasattr(self.estimator, "decision_function") and not hasattr(
            ...
            raise AttributeError(
                f"The provided estimator must have either a 'decision_function' method or 'predict_proba' method. You provided {self.estimator}"
            )
    ...
    def decision_function(self, X):
        ...
        if not hasattr(self.estimator, "decision_function"):
            raise AttributeError(
                f"You are calling 'decision_function' when the base estimator does not implement this method. Please try 'predict_proba' instead if you would like continuous outputs or ensure your estimator {self.estimator} implements the desired behavior."
            )

        y_sample_scores = self.estimator.decision_function(X)
        y_individual_scores = y_sample_scores.groupby(level=self.aggregation_index).agg(
            self._aggregation
        )
    ...
    def predict_proba(self, X):
        ...
        if not hasattr(self.estimator, "predict_proba"):
            raise AttributeError(
                f"You are calling 'predict_proba' when the base estimator does not implement this method. Please try 'predict_proba' instead if you would like continuous outputs or ensure your estimator {self.estimator} implements the desired behavior."
            )
...
class ThresholdPostprocessor(ClassifierMixin, BaseEstimator):
    ...
    def predict(self, X, **predict_params):
        ...
        if self.threshold_type == ThresholdType.decision_function:
            return (
                self.estimator.decision_function(X, **predict_params) >= self.threshold
            ).astype(int)
        elif self.threshold_type == ThresholdType.predict_proba:
            return (
                self.estimator.predict_proba(X, **predict_params) >= self.threshold
            ).astype(int)
        else:
