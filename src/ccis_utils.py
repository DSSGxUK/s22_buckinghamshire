...
def split_up_birth_month_year(df, inplace=False):
    ...
    return d.safe_drop_columns(df, [CCISDataColumns.month_year_of_birth], inplace=inplace)
...
def compute_age(df):
    ...
    """
    school_start = pd.to_datetime((df[CharacteristicsDataColumns.year] - 1).astype(pd.StringDtype()) + '-08-31') # gets the date of the end of the previous school year
    school_start.name = 'end'
    birth_date = pd.to_datetime(df[CharacteristicsDataColumns.birth_year].astype(pd.StringDtype()) + '-' + df[CharacteristicsDataColumns.birth_month].astype(pd.StringDtype()) + '-01') #gets the birth date and month
    ...
    if len(end_start_masked) == 0:
        ...
        return d.empty_series(len(end_start), name=CCISDataColumns.age, index=end_start.index).astype(pd.Int16Dtype())
    return end_start_masked.apply(lambda r: relativedelta(r['end'], r['start']).years, axis=1).reindex_like(end_start).astype(pd.Int16Dtype())
