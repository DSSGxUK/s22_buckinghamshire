...
def merge_priority_data(df1, df2, on, how='outer', unknown_vals: Optional[Dict] = None, na_vals=tuple()):
    ...
    for col in to_drop_cols:
        ...
        df[orig_col] = df[orig_col].replace(replacement_dict) \
            .fillna(df[col].replace(replacement_dict)) \
            .fillna(df[orig_col])  
    df = df.drop(to_drop_cols, axis=1)
    return df

def merge_priority_dfs(dfs, on, how='outer', unknown_vals: Optional[Dict] = None, na_vals=tuple()):
    df = dfs[0]
    for other_df in dfs[1:]:
        df = merge_priority_data(df, other_df, on=on, how=how, unknown_vals=unknown_vals, na_vals=na_vals)
    return df