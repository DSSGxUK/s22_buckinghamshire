...
def expand_categorical_max_agg_dict(
    ...
    categorical_max_aggs = [
        col for col, agg_type in agg_dict.items() if agg_type == Aggregations.categorical_max
    ]
    df_catcols = []
...
def build_agg(
    ...
    if agg == Aggregations.last_with_unknown:
        return partial(aggregate_last_with_unknown, unknown_codes=unknown_codes, na_vals=na_vals)
    else:
