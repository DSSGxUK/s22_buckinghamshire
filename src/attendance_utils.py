...
def keep_column_criteria(col_name, gby_columns):
    ...
    if col_name in asdict(NonabsenceReasons).values():
        return True
    if col_name in [AttendanceDataColumns.late_present, AttendanceDataColumns.termly_sessions_possible]:
        return True
    if col_name in [AttendanceDataColumns.total_absences, AttendanceDataColumns.total_nonabsences]:
        return True
