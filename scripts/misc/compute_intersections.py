...
all_merged_dfs = {k: pd.read_csv(fp, keep_default_na=False, dtype=str) for k, fp in constants.MERGED_FPS.items()}
all_dfs = flatten_dict(
    {k: extract_ind_dfs(mdf, constants.YEAR_COLS[k]) for k, mdf in all_merged_dfs.items()},
)
if DEBUG:
    all_dfs = {k: v for k,v in list(all_dfs.items())[:13]}

all_upns = {k:data_utils.get_unique_nonna(d, constants.UPN) for k, d in  all_dfs.items()}
...
for chunk in trange(0, combo_masks.shape[0], CHUNK_SIZE):
    combo_uniques = np.all(
        combo_masks[chunk:chunk + CHUNK_SIZE,..., None] & bin_masks[None, ...] | ~combo_masks[chunk:chunk + CHUNK_SIZE,..., None],
        axis=1
    )
    # Masks
...
df = pd.DataFrame(combo_unique_counts[:, None], index=mask_names, columns=['shared_upns'])
