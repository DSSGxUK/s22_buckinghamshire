...
def build_transition_df2(filtered_events_df):
    ...
    with Timer(text="filtered out sources in {:.8f}s"):
        source_df = filtered_events_df.loc[
            upn_event_groups.transform(mask_last).astype(bool)
        ]
    with Timer(text="filtered out targets in {:.8f}s"):
        target_df = filtered_events_df.loc[
            upn_event_groups.transform(mask_first).astype(bool)
        ]
    ...
    with Timer(text="filtered out targets in {:.8f}s"):
        transition_df = (
            transition_df.groupby(by=["source", "target"]).sum().reset_index()
        )

    labels = pd.concat([transition_df["source"], transition_df["target"]]).unique()

    transition_df = transition_df.replace({label: i for i, label in enumerate(labels)})
...
def build_sankey(transition_df, labels, min_weight=10):
    ...
    fig.update_layout(
        title_text="Trajectories of Students in Buckinghamshire", font_size=10
    )
...
if __name__ == "__main__":
    ...
    type_filter = ["post_16_activity", "compulsory_schooling"]
    events_df_mask = None
    ...
    with Timer(text="created joined events in {:.8f}s"):
        filtered_joined_events_df = pd.merge(
            filtered_events_df, single_upns_df, on=c.UPN, how="left"
        )
