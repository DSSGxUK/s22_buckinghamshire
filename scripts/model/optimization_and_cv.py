...
parser.add_argument('--space', required=True,
                    help='which search space to use', choices=list(cv.SEARCH_SPACES.keys()))
parser.add_argument('--parallel', action='store_true',
                    help='Whether or not to use parallelization')
parser.add_argument('--num_thresholds', type=int, required=False,
...
if __name__ == '__main__':
    ...
    pipeline = cv.PandasEstimatorWrapper(pipeline, preprocessor=preprocessor, postprocessor=postprocessor)
    ...
    _objective = cv.ObjectiveFunction(
        ...
        results_csv_fp=f.tmp_path(f.get_cv_results_filepath(args.space), debug=args.debug) if args.log_results else None,
        maximize=True
    )
    ...
    callbacks = []
    if args.checkpoint:
        callbacks.append(skopt.callbacks.CheckpointSaver(f.tmp_path(f.get_checkpoint_filepath(args.space), debug=args.debug)))
    if args.debug:
    ...
    if not args.single:
        search_space_dict = {**search_space_dict, "postprocessor__aggregation_method": cv.AGGREGATION_METHODS, "postprocessor__aggregation_index": [UPN]}
