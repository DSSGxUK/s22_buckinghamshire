...
parser.add_argument('--input', required=True,
                    help='where the input test csv is located')
parser.add_argument('--output', required=True,
                    help='where to output the evaluation metrics csv')
parser.add_argument('--model_output', required=True,
                    help='where to save the final model pkl file')
parser.add_argument('--model_pkls', required=True, nargs="+",
...
if __name__ == '__main__':
  ...
  # Load the test dataset
    ...
    for FP in MODEL_FP :
        ...
        model_dict = {"estimator": pipeline.estimator, "threshold": pipeline.threshold, "threshold_type" : pipeline.threshold_type}  
    ...
    pd.DataFrame({k: [v] for k, v in final_model_test_results.items()}).to_csv(TEST_RESULTS_CSV_FP, index=False)       
