...
parser.add_argument('--ks2', required=False,  # This can be None, in which case we won't use
...
parser.add_argument('--output_dataset_type', required=True, choices=asdict(OutputDatasetTypes).values(),
                    help='What kind of output merged dataset to build')
# Optional in case we don't have it
parser.add_argument("--chars", required=False, help="where to find the input characteristics data")
parser.add_argument('--target', required=False, choices=list(asdict(Targets).values()),
...
def school_year_of_age(df):
    ...
    school_year[sept_mask] = df.loc[sept_mask, CCISDataColumns.birth_year] + (df[AGE] + 1)
...
if __name__ == '__main__':
    ...
    if args.ks2 is not None:
        ...
        ks2_df = pd.DataFrame(columns=list(asdict(KS2Columns).values()) + list(asdict(PupilDeprivationColumns).values()))
    ...
    if args.output_dataset_type == OutputDatasetTypes.prediction and args.chars is not None:
        chars_df = d.load_csv(
            args.chars, 
            drop_empty=False, 
            drop_single_valued=False,
        ...
        chars_df = pd.DataFrame(columns=d.expand_categorical_columns(asdict(CharacteristicsDataColumns).values(), neet_df.columns, prefix_sep=CATEGORICAL_SEP))


    ### UPN Filter
    neet_df[[CCISDataColumns.compulsory_school_always, CCISDataColumns.neet_ever, CCISDataColumns.unknown_ever, CCISDataColumns.unknown_currently]] = \
        neet_df[[CCISDataColumns.compulsory_school_always, CCISDataColumns.neet_ever, CCISDataColumns.unknown_ever, CCISDataColumns.unknown_currently]].astype(pd.Int8Dtype()).astype(pd.BooleanDtype())

    if args.output_dataset_type == OutputDatasetTypes.modeling:
        logger.info(f"Output dataset type is {OutputDatasetTypes.modeling} so only keeping students data after compulsory school and either a neet activity or no unknown activity")
        neet_df = neet_df[~neet_df[CCISDataColumns.compulsory_school_always]]
        if args.target == Targets.neet_ever:
            neet_df = neet_df[neet_df[CCISDataColumns.neet_ever] | ~(neet_df[CCISDataColumns.unknown_ever])]
        elif args.target == Targets.neet_unknown_ever:
            neet_df[CCISDataColumns.neet_unknown_ever] = (neet_df[CCISDataColumns.neet_ever] | neet_df[CCISDataColumns.unknown_ever])
        ...
        other_upns = set(census_df[UPN].unique()) | set(att_df[UPN].unique()) | set(ks2_df[UPN].unique())
        neet_df = neet_df[neet_df[UPN].isin(other_upns)]


    elif args.output_dataset_type == OutputDatasetTypes.unknowns:
        logger.info(f"Output dataset type is {OutputDatasetTypes.unknowns} so only keeping students data are currently unknown")
        ...
        logger.info(f"Output dataset type is {OutputDatasetTypes.prediction} so only keeping students who are in the current census or attendance data and not in ccis data")
        ...
        without_ccis_upns = pd.DataFrame(list((set(census_df[UPN]) | set(att_df[UPN])) - set(neet_df[UPN])), columns=[UPN])
    ...
    ### Merge dataframes
    logger.info("Annotating dataframes with tags for tracking which students have what data")
    ...
    if args.output_dataset_type == OutputDatasetTypes.prediction:
        logger.info("Merging in priority order characteristics, census, attendance, ks2, ")
        merge_list = [chars_df, census_df, att_df, ks2_df]
    elif args.output_dataset_type in [OutputDatasetTypes.unknowns, OutputDatasetTypes.modeling]:
        logger.info("Merging in priority order 1. ccis, 2. census, 3. attendance, 4. ks2")
    ...
    for col in [CensusDataColumns.has_census_data, AttendanceDataColumns.has_attendance_data, KSDataColumns.has_ks2_data, CCISDataColumns.has_ccis_data, CharacteristicsDataColumns.has_characteristics_data]:
    ...
    merged_df[YEAR] = merged_df[YEAR].fillna(school_year_of_age(merged_df))  # Get years from known ages, birth_month, birth_year
    for col in [YEAR, CharacteristicsDataColumns.birth_year, CharacteristicsDataColumns.birth_month]:
        merged_df[col] = merged_df[col].astype(pd.Int16Dtype())
    merged_df[AGE] = merged_df[AGE].fillna(nu.compute_age(merged_df))  # Get ages from known years, birth_month, birth_year
    ...
    if args.output_dataset_type in (OutputDatasetTypes.unknowns, OutputDatasetTypes.prediction):
        logger.info("Building an inference dataset so getting rid of any target columns and other ccis columns")
        merged_df = d.safe_drop_columns(merged_df, columns=non_prediction_columns, inplace=True)
