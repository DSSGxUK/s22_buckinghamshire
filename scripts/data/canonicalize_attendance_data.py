...
# DVC Params
from src.constants import ATTENDANCE_COLUMN_RENAME, NA_VALS
...
parser.add_argument(
    "--input_new", required=True, help="where to find the input new attendance csv"
)
parser.add_argument(
    "--input_old", required=True, help="where to find the input old attendance csv"
)
parser.add_argument(
    "--output",
    required=True,
...
if __name__ == "__main__":
    ...
    att_dfs = {
        k: df.rename(columns=ATTENDANCE_COLUMN_RENAME) for k, df in att_dfs.items()
    }
    logger.info("Removing empty column code_f from attendance dataset")
    att_dfs = {
        k: df.drop("code_f", axis=1) if "code_f" in df.columns else df
