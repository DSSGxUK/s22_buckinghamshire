...
parser.add_argument('--single', action='store_true', help='should use single upn dataset instead of multi?')
parser.add_argument('--input', required=True,
                    help='where to find the prediction dataset')
parser.add_argument('--output', required=True,
                    help='where to save the feature selected dataset')
parser.add_argument('--student_names', required=True,
                    help='where to find the dataset with student names')
parser.add_argument('--unidentified_csv', required=True,
                    help='where to save the csv with unidentifiable students')
parser.add_argument('--train_data', required=True,
                    help='where to locate the training dataset')
parser.add_argument('--fill_fsme', action='store_true',
                    help='whether to forward fill fsme')
...
if __name__ == "__main__":
    ...
    df = d.get_dummies_with_logging(df, columns=[CensusDataColumns.fsme_on_census_day], logger=logger)
    breakpoint()
    
    logger.info(f'Selecting columns from the {"single" if args.single else "multi"} upn predictions dataset')
    ...
    breakpoint()
    logger.info("Add any columns that are in the train data but not in the prediction data")
    for col in features_for_model:
        if col not in df.columns:
            if not d.is_categorical(col):
                logger.warning(f"{col} is not in prediction dataset, but is not categorical. Filling in 0 to suppress error")
            df[col] = 0
    ...
    #breakpoint()
    add_data = df_additional_data[[UPN,"forename","preferred_surname","establishment_name","nc_year_actual","age"]]
