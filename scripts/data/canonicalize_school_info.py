...
# DVC Params
from src.params import SCHOOL_INFO_RENAME, NA_VALS, SchoolInfoColumns
...
parser.add_argument(
    "--input", required=True, help="where to find the input school info csv"
)
parser.add_argument(
    "--output",
    required=True,
...
if __name__ == "__main__":
    ...
    can_schools_df[SchoolInfoColumns.establishment_electoral_wards] = can_schools_df[
        SchoolInfoColumns.establishment_electoral_wards
    ].apply(lambda x: parse_human_list(x))
    # Remove the "- The" at the end of some school names
    can_schools_df[SchoolInfoColumns.establishment_name] = can_schools_df[
