...
parser.add_argument('--input', required=True,
                    help='where to find the input ccis neet csv')
...
if __name__ == "__main__":
    ...
    neet_dfs['feb18'][CCISDataColumns.review_date] = neet_dfs['feb18'][CCISDataColumns.review_date].apply(lambda x: d.read_excel_date(x) if not d.isna_scalar(x) else x)
    neet_dfs['feb18'][CCISDataColumns.neet_start_date] = neet_dfs['feb18'][CCISDataColumns.neet_start_date].apply(lambda x: d.read_excel_date(x) if not d.isna_scalar(x) else x)
