...
def merged_census_validation(df):
    assert not d.isna(
        df[CensusDataColumns.data_date], na_vals=NA_VALS
    ).any(), f"Some data dates are missing. Please check the input census file and ensure there are no entries with value {NA_VALS} in the data_date column. Then check the merge_data script for any bugs."
    try:
...
if __name__ == "__main__":
    ...
    logger.debug(
        f"{CensusDataColumns.fsme_on_census_day} currently has values {df[CensusDataColumns.fsme_on_census_day].unique()}"
    )
    ...
    logger.debug(
        f"{CensusDataColumns.fsme_on_census_day} now has values {df[CensusDataColumns.fsme_on_census_day].unique()}"
    )
