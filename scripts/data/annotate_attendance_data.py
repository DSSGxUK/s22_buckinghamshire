...
# DVC Params
from src.constants import AttendanceDataColumns, UNKNOWN_CODES, NA_VALS
...
def add_absence_columns(df, inplace=False):
    ...
    assert (
        AttendanceDataColumns.termly_sessions_authorised not in authorised_columns
    )  # This is not an "absence reason"
    unauthorised_columns = a.get_unauthorised_reason_columns(df)
    assert (
        AttendanceDataColumns.termly_sessions_unauthorised not in unauthorised_columns
    )  # This is not an "absence reason"
...
def term_end_to_term_type(term_ends):
    ...
    return months.astype(pd.StringDtype()).replace(
        {str(sum_month): "summer", str(aut_month): "autumn", str(spr_month): "spring"}
    )
...
def add_term_columns(df, inplace=False):
    ...
    df[AttendanceDataColumns.term_end] = pd.to_datetime(
        df[AttendanceDataColumns.data_date], errors="raise"
    )  # Attendance data date corresponds to the term end date
    ...
    df = d.add_column(
        ...
        get_term_end_year(
            term_types=df[AttendanceDataColumns.term_type],
            term_ends=df[AttendanceDataColumns.term_end],
        ),
        inplace=inplace,
    )
...
def attendance_merged_validation(df: pd.DataFrame):
    assert (
        df[AttendanceDataColumns.data_date].isna().any() == False
    ), "data_date column of attendance merged dataframe has null values, please ensure there is no error in your input to attendance_merged.py or any bug in that code."
