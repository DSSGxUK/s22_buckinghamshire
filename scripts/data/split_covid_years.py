...
def split_data(df, logger=l.PrintLogger, YEAR_OF_COVID_SPLIT) :
     """
    ...
    upns = []
    df_max_age_index = df.groupby(UPN).agg({AGE:'idxmax'}) # takes index of max age for each student (should be mostly 14)
    df_max_age_rows = df.iloc[df_max_age_index[AGE].values] # gets rows of max age for each student
    for val in df_max_age_rows[AGE].unique() : #loop through ages
        covid_years = df_max_age_rows[(df_max_age_rows[AGE]==val) & (df_max_age_rows[YEAR]>=(val+(year_age_difference)))] # identify students affected by covid (if students are 14 in or after 2018)
        upns.append(covid_years[UPN]) # get upns of these students 
    
    df_all_upns = reduce(lambda df1,df2: pd.merge(df1,df2,how='outer',on=UPN), upns) #merge all upns into one dataframe
    
    post_df = df[df[UPN].isin(df_all_upns[UPN])] #select all students that were affected by covid
    pre_df = df[~df[UPN].isin(df_all_upns[UPN])] #select all students that were not affected by covid
...
parser.add_argument('--output_pre_covid', required=True,
                    help='where to put pre-covid multi upn dataset')
parser.add_argument('--output_post_covid', required=True,
...
if __name__ == "__main__":
    ...
    pre_df, post_df = split_data(df, logger=l.PrintLogger, YEAR_OF_COVID_SPLIT) 
