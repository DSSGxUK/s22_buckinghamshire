...
parser.add_argument('--ccis_input', required=True,
                    help='where to find the CCIS (NEET dataset) annotated csv')
parser.add_argument('--ks_input', required=True,
                    help='where to find the Key Stage annotated csv')
parser.add_argument('--output', required=True,
                    help='where to put the produced CCIS - Key Stage EDA csv')
...
if __name__ == '__main__':
    ...
    # Set up logging
    logger = l.get_logger(name=f.get_canonical_filename(__file__), debug=args.debug, file_logging=args.no_file_logging)
    ...
    neet_df = neet_df.loc[~(neet_df[CCISDataColumns.compulsory_school].astype(bool))].drop(CCISDataColumns.compulsory_school, axis=1)
    logger.info(f"Grouping the ccis data by UPN to get a dataframe of students mapped to their post-16 statuses")
    ...
    logger.info(f"Filtering out {ks_filter} columns from the key stage annotated dataset.")
