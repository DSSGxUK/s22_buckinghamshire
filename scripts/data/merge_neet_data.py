...
parser.add_argument('--input', required=True,
                    help='where to find the input canonicalized ccis data csv')
parser.add_argument('--output', required=True,
                    help='where to put the output merged ccis csv')
...
if __name__ == '__main__':
    ...
    neet_dfs = {d.savedate_to_datetime(date) + relativedelta(months=1): df for date, df in neet_dfs.items()}
    merged_neet_df = pd.concat(
        [d.add_column(df, CCISDataColumns.ccis_period_end, datetime.strftime(date, '%Y-%m-%d')) for date, df in sorted(neet_dfs.items(), key=lambda x: x[0])], 
        axis=0
    )
