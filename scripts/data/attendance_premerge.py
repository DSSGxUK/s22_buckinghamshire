...
def create_attendance_percent1_df(
    df, 
    ...
    logger.info(f'Computing percentage of total_nonabsences wrt termly_sessions_possible + total_nonabsences')
    df_percent[au.AttendanceDataColumns.total_nonabsences] = d.divide_with_na(
        df[au.AttendanceDataColumns.total_nonabsences], 
        df[au.AttendanceDataColumns.total_nonabsences] +  df[au.AttendanceDataColumns.termly_sessions_possible], 
        fillna=0, 
        fill_prev_na=False
    )
    
    logger.info(f'Computing percentage of cols {nonabsence_columns} wrt total_nonabsences')
    ...
    df_percent = df_percent.drop(au.AttendanceDataColumns.termly_sessions_possible, axis=1)
    
    return df_percent

def create_attendance_percent2_df(
    df,
    logger=l.PrintLogger
):
    """
    ...
    logger.info('Compute percentage of sessions possible the student attended')
    df_percent = df.copy()
    by_session_cols = [AttendanceDataColumns.total_absences, AttendanceDataColumns.late_present] + absence_columns
    logger.info(f'Computing percentage for cols {by_session_cols} wrt termly_sessions_possible')
    ...
    # Percent of total sessions spent in nonabsence
    by_total_sessions_cols = [AttendanceDataColumns.total_nonabsences] + nonabsence_columns
    logger.info(f'Computing percentage for cols {by_total_sessions_cols} wrt termly_sessions_possible + total_nonabsences')
...
def create_attendance_normed_df(df, eps=1e-9, logger=l.PrintLogger):
    ...
    """
    gby_columns = [AttendanceDataColumns.upn, AttendanceDataColumns.year, AttendanceDataColumns.term_type]
    ...
    df_with_norms = df.merge(df_term_means, on=norm_gby_columns, suffixes=(None, '_mean'), how='left') \
