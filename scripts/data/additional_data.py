...
def create_attendance_exact_df(df, logger=l.PrintLogger):
    ...
    drop_columns = [
        c for c in df.columns if not au.keep_column_criteria(c, gby_columns)
    ]
    ...
    df.drop(
        df[df["total_absences"] > df["termly_sessions_possible"]].index, inplace=True
    )  # drop rows in which total absences is greater than termly sessions possible
...
if __name__ == "__main__":
    ...
    if args.single:
        ...
        logger.info(
            f"Saving columns {extra_columns} from {args.census_input} to an additional dataset"
        )

        extra_cols_df = df[extra_columns]
        extra_cols_df = extra_cols_df.sort_values(
            by=[CensusDataColumns.census_period_end, CensusDataColumns.nc_year_actual]
        )
        ...
        census_agg = (
            extra_cols_df.groupby(UPN)
            .agg(
                {
                    "postcode": "last",
                    "forename": "last",
                    "preferred_surname": "last",
                    "establishment_name": "last",
                    "establishment_number": "last",
                    "establishment_postcode": "last",
                    "la_establishment_number": "last",
                    "nc_year_actual": "max",
                    "age": "max",
                }
            )
        ...
        logger.info(
            f"Aggregating number of days absent for each student {args.att_input}"
        )

        att_agg = (
            att_exact.groupby(UPN).agg({"excluded_authorised": "sum"}).reset_index()
        )
        # att_exact[]
        ...
        merged = merged.rename(
            columns={"excluded_authorised": "excluded_authorised_exact"}
        )
        # breakpoint()
