...
def test_build_pipeline_basic():
    ...
    actual_pipeline = cv.build_pipeline(sample_pipeline_steps, model_constructors=mock_model_constructors)
...
def test_build_search_space_basic():
    sample_search_space = [({ 
            ...
            'model__l1_ratio': (0., 1., 'uniform', ParameterSpaceTypes.real),
    ...
    pipeline_steps = [('model', Models.logistic_regression), ('oversampling', OversamplingMethods.none)]

    actual_search_space = cv.build_search_space(sample_search_space, pipeline_steps, model_constructors=mock_constructors, parameter_space_types=cv.PARAMETER_SPACE_TYPES)
...
def test_get_pipeline_params():
    pipeline = Pipeline([('scaler', None), ('oversampling', SMOTEN()), ('model', LGBMClassifier())])
    pipeline.set_params(model__objective='binary', oversampling__k_neighbors=10)
    actual_params = cv.get_pipeline_params(pipeline)
    expected_params = {
        'model__objective': 'binary', 'oversampling__k_neighbors': 10, 
...
def test_threshold_scorer_get_threshold_range_no_filter_between_auto_buffer():
    ...
    y_score = np.array([-0.0, -1.9, 1.4, 0.0, 2.3, -0.8, 0.6, -0.0, -0.4, 2.6, -0.0, -1.9, 1.4, 0.9, -1.0, -0.8, 0.6, -0.0, -0.4, 2.6])
    ...
    expected = np.array(
            [-2.35, -1.45, -.9, -.6, -.2, .3, .75, 1.15, 1.85, 2.45, 2.75]
    )
...
def test_threshold_scorer_get_threshold_range_no_filter_weighted_between_auto_buffer():
    ...
    r = ts.get_threshold_range(
        y_score, 
        num_thresholds=None,
        method='weighted_between',
        boundary_buffer='auto'
    )
    [-1.9,  -1. , -0.8, -0.4, -0. ,  0.6,  0.9,  1.4,  2.3,  2.6]
    expected = np.array([
        -2.2,  
        -1.6, # -1.9, -1
        (-1 * 1 - .8 * 2) / 3, # -.8, -1
...
def test_threshold_scoring_decision_function():
    ...
    class Estimator:
        ...
        def decision_function(self, X):
            return np.array([-0.0, -1.9, 1.4, 0.0, 2.3, -0.8, 0.6, -0.0, -0.4, 2.6, -0.0, -1.9, 1.4, 0.9, -1.0, -0.8, 0.6, -0.0, -0.4, 2.6])
