...
def test_all_equal_all_nans():
    tdf = pd.DataFrame({'Animal': [np.nan, np.nan,
                              np.nan, np.nan],
                   'Max Speed': [26., 26., 26., 26.]})
...
def test_get_dummies_with_logging_basic() :
    ...
    test_col = ['Animal']
    test_prefix_sep = "__"
    expected = pd.DataFrame({'Max Speed': [26., 26., 26., 26.],
                              'Animal__Cow': [0,0,0,1],
    ...
    pd.testing.assert_frame_equal(data_utils.get_dummies_with_logging(tdf, test_col, prefix_sep=test_prefix_sep, logger=log_utils.PrintLogger), expected, check_dtype=False) #uint8 vs int64 dtypes?
...
def test_get_dummies_with_logging_nans() :
    ...
    pd.testing.assert_frame_equal(data_utils.get_dummies_with_logging(tdf, test_col, prefix_sep=test_prefix_sep, logger=log_utils.PrintLogger), expected, check_dtype=False)
...
def test_drop_duplicate_rows_basic(): 
    tdf = pd.DataFrame({'Animal': ['Falcon', 'Falcon',
                                  'Fox', 'Cow'],
                       'Max Speed': [26., 26., 26., 26.]})
    expected = pd.DataFrame({'Animal': ['Falcon',
                                  'Fox', 'Cow'],
                       'Max Speed': [26.,26.,26.]},index=[0,2,3]) #should the indexes be reset?

    pd.testing.assert_frame_equal(data_utils.drop_duplicate_rows(tdf, logger=log_utils.PrintLogger),expected)
...
def test_drop_empty_cols_basic():
    ...
    pd.testing.assert_frame_equal(data_utils.drop_empty_cols(tdf,na_vals=NA_VALS, logger=log_utils.PrintLogger),expected)    
...
def test_drop_single_valued_cols_basic():
    ...
    expected = pd.DataFrame({'Animal': ['Falcon', 'Falcon',
                                  'Fox', 'Cow'],
                        'Number of legs':[2,2,4,4]})  
    pd.testing.assert_frame_equal(data_utils.drop_single_valued_cols(tdf,logger=log_utils.PrintLogger),expected)    
    
def test_drop_empty_upns_basic():
    tdf = pd.DataFrame({'Animal': [np.nan, 'Falcon',
                                  'Fox', 'Cow'],
                       'Max Speed': [26., 26., 26., 26.],
                        'Number of legs':[2,2,4,4]},index=[0,1,2,3]
                       )
    expected = pd.DataFrame({'Animal': ['Falcon',
                                  'Fox', 'Cow'],
                        'Max Speed': [26., 26., 26.],
                        'Number of legs':[2,4,4]}, index=[1,2,3])  
    upn_col = 'Animal'
    NA_VALS = ['', '#VALUE!'] #must be a list
    print (expected)
    print (data_utils.drop_empty_upns(tdf, upn_col, na_vals=NA_VALS,logger=log_utils.PrintLogger))

    pd.testing.assert_frame_equal(data_utils.drop_empty_upns(tdf, upn_col, na_vals=NA_VALS,logger=log_utils.PrintLogger),expected)
