...
from src.constants import CCISDataColumns

SINGLE_INPUT_DATA_FP = "test_data/test_split_data_single_input.csv"
EXPECTED_SINGLE_TRAIN_OUTPUT_DATA_FP = (
    "test_data/test_split_data_single_train_output.csv"
)
...
def test_single_split_data_basic():
    call(
        [
            ...
            "--input",
            SINGLE_INPUT_DATA_FP,
            "--split",
            "0.2",
            "--test_output",
            ACTUAL_SINGLE_TEST_OUTPUT_DATA_FP,
            "--train_output",
            ACTUAL_SINGLE_TRAIN_OUTPUT_DATA_FP,
            "--seed",
            "1",
            "--no_file_logging",
            "--single",
    ...
    expected_train_output = pd.read_csv(
        EXPECTED_SINGLE_TRAIN_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
    expected_test_output = pd.read_csv(
        EXPECTED_SINGLE_TEST_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
    actual_train_output = pd.read_csv(
        ACTUAL_SINGLE_TRAIN_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
    actual_test_output = pd.read_csv(
        ACTUAL_SINGLE_TEST_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
...
def test_multi_split_data_basic():
    call(
        [
            ...
            "0.2",
            "--test_output",
            ACTUAL_MULTI_TEST_OUTPUT_DATA_FP,
            "--train_output",
            ACTUAL_MULTI_TRAIN_OUTPUT_DATA_FP,
            "--seed",
            "1",
            "--no_file_logging",
            "--target",
    ...
    expected_train_output = pd.read_csv(
        EXPECTED_MULTI_TRAIN_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
    expected_test_output = pd.read_csv(
        EXPECTED_MULTI_TEST_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
    actual_train_output = pd.read_csv(
        ACTUAL_MULTI_TRAIN_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
    actual_test_output = pd.read_csv(
        ACTUAL_MULTI_TEST_OUTPUT_DATA_FP, keep_default_na=False
    ).convert_dtypes()
